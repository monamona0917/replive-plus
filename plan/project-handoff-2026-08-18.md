...
## 12. 当前状态补充（2026-08-18）
 
本节是本次更新的权威补充；前面章节保留 2026-08-13 的完整历史记录。
 
### 12.1 分支、目录和未提交改动
 
- 当前分支为 cleanup-old-web，远程为 https://github.com/monamona0917/nsy_chat_live-plus.git。
- 当前前端只有 replive-web-pro/；旧 replive-web/ 已清理。工作树中删除旧构建入口、前端预览/提取脚本的改动仍属于用户未提交改动，不要恢复。
- 当前还存在 handler、ChatList、MessageBubble、MediaGalleryDrawer、chat-store、chat 类型以及两个计划文件的未提交改动。先执行 git status --short，再处理任何新任务。
- 本次没有重新构建、启动程序或推送；用户希望自己负责构建。vendor/ 由当前分支历史保留，构建优先使用 -mod=vendor。
 
### 12.2 当前同步行为
 
- 主程序启动时初始化登录、API、SQLite、Fandom 本地媒体回填，然后由 service.Init 立即同步 Fandom 房间/消息和 Prime Chat；Prime 消息同步只在启动时执行，不加入周期 worker。
- Fandom saveChatRooms worker 的随机间隔为 29--58 秒，refreshNewMessages 为 3--6.9 秒，关注对象资料为 10 分钟。前端 Fandom 新消息轮询为 15 秒；历史窗口 hasNewer=true 时 Store 会暂停它。
- 主后端不再轮询直播状态或启动 ffmpeg。录像只通过 cmd/replive-live-recorder/ 独立进程运行，默认 -interval 2s。
- scheduled_chat_message、定时发送和付费 card 已移除；不要再从 config 或 UI 恢复。
 
### 12.3 当前聊天跳转和接口
 
- handler/chat_handler.go 与 handler/prime_chat_handler.go 的 around 以时间加 id 为复合顺序，返回 anchor 前后窗口并保证 anchor 在页内；Fandom 日期查询兼容秒、毫秒和 time_str。
- chat-store.ts 的日期跳转优先使用内存目标，否则沿 older 游标每页 100 条向前加载；相册/关键词跳转先 around，再沿 newer 每页 100 条补齐到 hasNewer=false。不使用前端固定 200 页上限，但检查重复/不前进游标防死循环。
- JumpTarget 在请求发起时立即设置 roomKey、backendId、requestId；roomEpoch 和 requestId 防止旧请求覆盖切房或新跳转。
- ChatList.tsx 在跳转时保留 DOM、拦截自动分页和贴底，用 MutationObserver 等待节点，用 ResizeObserver 补偿图片高度，瞬时居中，成功后高亮约 1.5 秒并按 requestId 释放锁。日期、搜索、相册三种跳转必须分别回归。
- “加载后续较新消息”按钮和普通触底 newer 已移除，因为相册/关键词跳转会主动补齐到最新；普通历史浏览仍能向上加载 older。
- service/chat.go 的后端完整历史同步仍有 maxChatMessagePages=200，这是后端同步保护，不是前端跳转补齐上限，不能混淆。
 
### 12.4 当前媒体和时间规则
 
- Fandom 聊天图片和相册缩略图不使用原生 loading=lazy；相册首批有限渲染并限制图片并发。聊天/相册视频使用 metadata 预加载，仍需检查 CDN 首帧和黑屏。
- Fandom 前端优先 /media/... 本地 URL，404 或加载失败时只回退一次远程 URL；Prime 不接入本地媒体映射。
- Go 与前端统一 Asia/Tokyo。Fandom 的 talent_last_check_time 用于 Fandom 我方消息的 既読显示；Prime 字段 100/101 仍只能作诊断，不得称作 Prime 已读。
 
### 12.5 当前手动构建
 
前端必须先生成 replive-web-pro/dist，之后再构建 go:embed Web 程序：
...
中国网络环境依赖下载使用 GOPROXY=https://goproxy.cn,direct 和 GOSUMDB=sum.golang.google.cn。除非用户明确授权，Agent 不启动程序、不覆盖 dist 中无关产物、不擅自构建。
 
### 12.6 文档维护
 
以后维护交接说明时，按更新当天创建 project-handoff-YYYY-MM-DD.md，并在 plan/README.md 指向最新文件；旧日期文件作为历史快照保留。每次行为、协议、数据库、媒体、轮询或构建变化，都同步维护本文件和 chat-ui-multi-room-cursor-translation.md。源码当前行为优先于历史计划文字。
